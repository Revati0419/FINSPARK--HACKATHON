...
function init() {
  chrome.storage.sync.get(['isBarEnabled', 'theme'], (result) => {
      ...
      // Apply theme regardless of bar visibility for consistency
...
function createFloatingBar() {
  if (document.getElementById('assistant-pro-bar')) return; // Already exists
...
function createAiPanel() {
  if (document.getElementById('assistant-pro-ai-panel')) return; // Already exists
...
function addBarEventListeners() {
  document.getElementById('assistant-translate-btn').addEventListener('click', () => {
      ...
      // Backend logic for full page translation will go here.
  });
  ...
  document.getElementById('assistant-audio-btn').addEventListener('click', () => {
      ...
      // Logic to toggle audio on/off in chrome.storage
  });
...
function applyTheme(theme) {
  ...
  if (theme === 'night') {
      document.body.classList.add('assistant-night-mode'); // Optional: for global page styling
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'toggleBar') {
      if (request.isEnabled) {
          init(); // This will create the UI if it doesn't exist
      } else {
          destroyUI();
      }
  }
